...
def main():
    ...
    # can be used to skip the evaluation with 'coco_eval'
    # which will return a precision numpy tensor that must
    # have been stored beforehand
    parser.add_argument('--load-precisions', action='store_true')
    args = parser.parse_args()

    # args.path e.g. where gt.json file is located
    print(f'Analyzing {args.path}')
    p = Path(args.path)
    assert p.exists() and p.is_dir()

    # glob_files uses args.path where either a single pred.json
    # or multiple pred_*.json files are located from different
    # runs e.g. from 'ntimes.sh' script
    ...
    # for each 'run' (= a pred_*.json file) we get a dictionary with
    # detailed description of different mAP values
